...
const data = [
 ...
 /* create an object that summarizes the incomplete tasks of users 3, 7, & 8
    ...
    ```*/
    const userTasks = data.filter(todo => (todo.userId === 3 || todo.userId === 7 || todo.userId === 8));

const tasksSummary = userTasks.reduce(
  (acc, curr) => {

    // check if object exists in accumulator, if not create one
    if (!acc[curr.userId]) {
      acc[curr.userId] = {
        totalTasks: 0,
        totalIncomplete: 0,
    ...
    // calculate
     ...
     acc[curr.userId].incompleteTasks.push(curr.title);
